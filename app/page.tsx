...
import { ArrowUpRight, ArrowRight, Flame, Plus, Clock3, Menu, X, ShoppingBag, Check } from 'lucide-react';
import { Dialog, DialogContent, DialogTitle, DialogDescription } from '@/components/ui/dialog';
type Panel = 'menu' | 'recipe' | 'order' | null;
export default function Home() {
  ...
  function open(next: Panel) { setPanel(next); setMobileOpen(false); }
  return <main className="site-shell">
    <a className="skip-link" href="#hero">Skip to content</a>
    <motion.header className="header" initial={reduce ? false : { opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }}>
      <a className="brand" href="/" aria-label="Ember and Bun home"><Flame aria-hidden="true" className="brand-flame" fill="currentColor"/><span>ember<span className="amp">&</span>bun<span className="brand-dot">®</span><small>SMASHED. SEARED. SERVED.</small></span></a>
      <nav className="pill-nav" aria-label="Main navigation"><a className="active" href="#hero" aria-current="page">Home</a><button onClick={() => open('menu')}>The burger</button><button onClick={() => open('recipe')}>Our recipe <ArrowUpRight size={14}/></button></nav>
      <button className="nav-order" onClick={() => open('order')}><span>Let’s eat</span><ShoppingBag size={19}/></button>
      <button className="mobile-toggle" aria-label={mobileOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={mobileOpen} onClick={() => setMobileOpen(!mobileOpen)}>{mobileOpen ? <X/> : <Menu/>}</button>
      {mobileOpen && <nav className="mobile-nav" aria-label="Mobile navigation"><button onClick={() => open('menu')}>The burger <ArrowUpRight/></button><button onClick={() => open('recipe')}>Our recipe <ArrowUpRight/></button><button onClick={() => open('order')}>Let’s eat <ShoppingBag/></button></nav>}
    </motion.header>
    <section id="hero" className="hero" aria-labelledby="hero-title" onPointerMove={e => { if (reduce || e.pointerType !== 'mouse') return; const r=e.currentTarget.getBoundingClientRect(); mx.set((e.clientX-r.left-r.width/2)*.017); my.set((e.clientY-r.top-r.height/2)*.017); }} onPointerLeave={() => { mx.set(0); my.set(0); }}>
    ...
    <Dialog open={panel !== null} onOpenChange={value => { if (!value) setPanel(null); }}>
      <DialogContent className="burger-dialog">
        <span className="dialog-kicker">EMBER & BUN / {panel === 'recipe' ? 'THE GOOD STUFF' : panel === 'order' ? 'YOUR SMASH' : 'THE HOUSE FAVOURITE'}</span>
        <DialogTitle className="dialog-title">{panel === 'recipe' ? 'ZERO SHORTCUTS.' : panel === 'order' ? 'GOOD TASTE. GREAT CHOICE.' : 'THE EMBER DOUBLE.'}</DialogTitle>
        <DialogDescription className="dialog-description">{panel === 'recipe' ? 'It starts with a hot griddle and ends with a very good mess.' : panel === 'order' ? 'Online ordering is coming soon. Here’s a little taste of what’s on the way.' : 'Crispy, juicy and unapologetically cheesy. Meet our signature smash.'}</DialogDescription>
        {panel === 'recipe' ? <ul className="recipe-list">{['Two beef patties, smashed thin for crispy edges','Melted American cheese on every layer','Tangy pickles, fresh onion and our house sauce','A soft, toasted sesame brioche bun'].map(t => <li key={t}><Check size={20}/>{t}</li>)}</ul> : <><div className="dialog-product"><img src="/smash-burger.png" alt="The Ember Double" width={220} height={220}/><div><strong>The Ember Double</strong><p>Double beef, American cheese, pickles, onion & Ember sauce.</p></div></div>{panel === 'order' ? <p className="order-note">Ordering isn’t live yet. No order has been placed.</p> : <button className="cta cta-primary" onClick={() => open('order')}>I’m hungry <ArrowUpRight/></button>}</>}
      </DialogContent>
    </Dialog>
  </main>;
}
